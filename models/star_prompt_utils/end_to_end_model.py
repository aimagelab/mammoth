...
class STARPromptModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cur_classes: int, frozen_past_classes=0) -> torch.Tensor:
        """
    ...
    @torch.no_grad()
    def eval_first_stage_on_task(self, dataset: ContinualDataset) -> torch.Tensor:
        """
    ...
    def train_first_stage_on_task(self, dataset: ContinualDataset, current_task: int, n_past_classes: int, n_seen_classes: int, loss_fn):
        ...
        if self.args.first_stage_optim == 'sgd':
            opt = torch.optim.SGD(self.first_stage.parameters(), lr=self.args.first_stage_lr, momentum=self.args.first_stage_momentum,
                                  weight_decay=self.args.first_stage_weight_decay)
        else:
            opt = torch.optim.Adam(self.first_stage.parameters(), lr=self.args.first_stage_lr,
                                   weight_decay=self.args.first_stage_weight_decay)
        ...
        cur_acc = self.eval_first_stage_on_task(dataset)
        print(f'First stage accuracy: {cur_acc}')
        print(f'\tAverage: {cur_acc.mean().item():.2f}')
