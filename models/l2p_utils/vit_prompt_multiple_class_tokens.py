...
from lifelong_methods.methods.l2p_utils.prompt import Prompt
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            top_k=None, batchwise_prompt=False, prompt_key_init='uniform', head_type='token', use_prompt_mask=False, many_cltoks=False):
        """
        Args:
        ...
        self.num_prefix_tokens = (num_classes if many_cltoks else 1) if class_token else 0
        ...
        if many_cltoks:
            self.cls_tokens = nn.Parameter(torch.zeros(1, num_classes, embed_dim)) if class_token else None
        else:
        ...
        self.heads = nn.ModuleList([nn.Linear(self.embed_dim, 1) if num_classes > 0 else nn.Identity() for _ in range(num_classes)])

    def init_weights(self, mode=''):
        assert mode in ('jax', 'jax_nlhb', 'moco', '')
        head_bias = -math.log(self.num_classes) if 'nlhb' in mode else 0.
        trunc_normal_(self.pos_embed, std=.02)
        if hasattr(self, 'cls_tokens') and self.cls_tokens is not None:
            nn.init.normal_(self.cls_tokens, std=1e-6)
        elif hasattr(self, 'cls_token') and self.cls_token is not None:
    ...
    def no_weight_decay(self):
        return {'pos_embed', 'cls_tokens', 'dist_token', 'cls_token'}

    @torch.jit.ignore
    def group_matcher(self, coarse=False):
        return dict(
            stem=r'^cls_token(s)?|pos_embed|patch_embed',  # stem and embed
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        if hasattr(self, 'cls_tokens') and self.cls_tokens is not None:
            x = torch.cat((self.cls_tokens.expand(x.shape[0], -1, -1), x), dim=1)
        elif hasattr(self, 'cls_token') and self.cls_token is not None:
    ...
    def forward_head(self, res, pre_logits: bool = False):
        x = res['x']
        if hasattr(self, 'cls_token') and self.cls_token is not None and self.head_type == 'token':
            x = x[:, 0]
        elif hasattr(self, 'cls_tokens') and self.cls_tokens is not None and self.head_type == 'token':
            x = x[:, :self.num_prefix_tokens]
            res['pre_logits'] = x
            x = self.fc_norm(x)
            ret = [head(x[:, i, :]) for i, head in enumerate(self.heads)]
            ...
            # TODO nota per marin: ma questo non dovrebbe essere "if not self.class_token"?
            # in realtà head_type=prompt significa che considero solo il prompt, e devo trascurare il class token se è presente
            # il codice originale era: x = x[:, 1:(1 + self.total_prompt_len)] if self.class_token else x[:, 0:self.total_prompt_len]
            x = x[:, self.num_prefix_tokens:(self.num_prefix_tokens + self.total_prompt_len)] if self.class_token else x[:, 0:self.total_prompt_len]
            x = x.mean(dim=1)
        elif self.head_type == 'token+prompt' and self.prompt_pool and self.class_token:
            raise NotImplementedError # TODO check
            x = x[:, 0:self.total_prompt_len + self.num_prefix_tokens]
            x = x.mean(dim=1)
        else:
...
def resize_pos_embed(posemb, posemb_new, num_prefix_tokens=1, gs_new=()):
    ...
    if ntok_new > gs_old ** 2:
        ntok_new -= gs_old ** 2
        # expand cls's pos embedding for pgrompt tokens
    ...
    for k, v in state_dict.items():
        if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
            ...
            v = resize_pos_embed(
                v,
                model.pos_embed,
                0 if getattr(model, 'no_embed_class') else v.shape[1] - torch.tensor(list(model.patch_embed.grid_size)).prod().item(), #getattr(model, 'num_prefix_tokens', 1), # old formula depended on new shape, not checkpoint shape
                model.patch_embed.grid_size
            )
