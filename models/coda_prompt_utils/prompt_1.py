...
class CodaPromptDifferentSize(nn.Module):
    ...
    def prepare_prompts(self, task_id):
        for e in self.e_layers:
            ...
            if task_id == 0:
                ...
                setattr(self, f'e_p_{e}', [nn.Parameter(p.to(self.device))]) # (pool_size, e_l, emb_d)
                setattr(self, f'e_k_{e}', [nn.Parameter(k.to(self.device))])
                setattr(self, f'e_a_{e}', [nn.Parameter(a.to(self.device))])
            else:
                new_pt = int(self.e_pool_size / (task_id+1))

                setattr(self, f'e_p_{e}', [nn.Parameter(torch.zeros(new_pt, e_l, self.emb_d).to(self.device)) for _ in range(task_id+1)])
                setattr(self, f'e_k_{e}', [nn.Parameter(torch.zeros(new_pt, self.key_d).to(self.device)) for _ in range(task_id+1)])
                setattr(self, f'e_a_{e}', [nn.Parameter(torch.zeros(new_pt, self.key_d).to(self.device)) for _ in range(task_id+1)])
        ...
        for e in self.e_layers:
            ...
            if task_id == 0:
                ...
                for t in range(task_id):
                    prompt_comp.append(getattr(self,f'e_p_{e}')[t][:new_pt])
                    keys.append(getattr(self,f'e_k_{e}')[t][:new_pt])
                    attn_vec.append(getattr(self,f'e_a_{e}')[t][:new_pt])
                # add new prompt
                prompt_comp.append(tensor_prompt(new_pt, e_l, self.emb_d, ortho=True).to(self.device))
                keys.append(tensor_prompt(new_pt, self.key_d, ortho=True).to(self.device))
                attn_vec.append(tensor_prompt(new_pt, self.key_d, ortho=True).to(self.device))

                setattr(self, f'e_p_{e}', [nn.Parameter(prompt_comp[t]) for t in range(task_id+1)])
                setattr(self, f'e_k_{e}', [nn.Parameter(keys[t]) for t in range(task_id+1)])
                setattr(self, f'e_a_{e}', [nn.Parameter(attn_vec[t]) for t in range(task_id+1)])
    
    def get_prompt_parameters(self):
        params = {}
        for e in self.e_layers:
            n = len(getattr(self,f'e_k_{e}'))
            for t in range(n):
                params[f'e_p_{e}_{t}'] = getattr(self,f'e_p_{e}')[t]
                params[f'e_k_{e}_{t}'] = getattr(self,f'e_k_{e}')[t]
                params[f'e_a_{e}_{t}'] = getattr(self,f'e_a_{e}')[t]
        return params
    ...
    def forward(self, x_querry, l, x_block, train=False, task_id=None):
        ...
        if l in self.e_layers:
            ...
            if train:
                ...
                old_K = [K[i].detach().clone() for i in range(task_id)]
                old_A = [A[i].detach().clone() for i in range(task_id)]
                old_p = [p[i].detach().clone() for i in range(task_id)]
