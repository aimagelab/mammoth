    ...
    def init_prompts(self, task_id):
        for e in self.e_layers:
            ...
            if task_id == 0:
                ...
                setattr(self, f'e_p_{e}', ParameterList([p.to(self.device)])) # (pool_size, e_l, emb_d)
                setattr(self, f'e_k_{e}',ParameterList([k.to(self.device)]))
                setattr(self, f'e_a_{e}',ParameterList([a.to(self.device)]))
            else:
                prompt_comp = []
                keys = []
                attn_vec = []
                ...
                setattr(self, f'e_p_{e}',ParameterList(prompt_comp))
                setattr(self, f'e_k_{e}',ParameterList(keys))
                setattr(self, f'e_a_{e}',ParameterList(attn_vec))
