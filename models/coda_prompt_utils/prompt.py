...
class CodaPromptReservoir(nn.Module):
    ...
    def prepare_prompts(self, task_id):
        for e in self.e_layers:
            ...
            if task_id == 0:
                ...
                setattr(self, f'e_p_{e}', nn.Parameter(p.unsqueeze(0).to(self.device))) # (pool_size, e_l, emb_d)
                setattr(self, f'e_k_{e}', nn.Parameter(k.unsqueeze(0).to(self.device)))
                setattr(self, f'e_a_{e}', nn.Parameter(a.unsqueeze(0).to(self.device)))
            else:
                new_pt = int(self.e_pool_size / (task_id+1))

                setattr(self, f'e_p_{e}', nn.Parameter(torch.zeros(task_id+1, new_pt, e_l, self.emb_d).to(self.device)))
                setattr(self, f'e_k_{e}',nn.Parameter(torch.zeros(task_id+1, new_pt, self.key_d).to(self.device)))
                setattr(self, f'e_a_{e}',nn.Parameter(torch.zeros(task_id+1, new_pt, self.key_d).to(self.device)))
    ...
    def init_prompts(self, task_id):
        for e in self.e_layers:
            ...
            if task_id == 0:
                ...
                setattr(self, f'e_p_{e}', nn.Parameter(torch.stack(prompt_comp)))
                setattr(self, f'e_k_{e}',nn.Parameter(torch.stack(keys)))
                setattr(self, f'e_a_{e}',nn.Parameter(torch.stack(attn_vec)))
