...
class CodaPromptReservoirNew(nn.Module):
        ...
        for e in self.e_layers:
            ...
            if task_id == 0:
                ...
                for t in range(task_id):
                    setattr(self, f'e_p_{e}_{t}', nn.Parameter(getattr(self,f'e_p_{e}_{t}')[:new_pt]))
                    setattr(self, f'e_k_{e}_{t}', nn.Parameter(getattr(self,f'e_k_{e}_{t}')[:new_pt]))
                    setattr(self, f'e_a_{e}_{t}', nn.Parameter(getattr(self,f'e_a_{e}_{t}')[:new_pt]))
                # add new prompt
                setattr(self, f'e_p_{e}_{task_id}', nn.Parameter(tensor_prompt(new_pt, e_l, self.emb_d, ortho=True).to(self.device)))
                setattr(self, f'e_k_{e}_{task_id}', nn.Parameter(tensor_prompt(new_pt, self.key_d, ortho=True).to(self.device)))
                setattr(self, f'e_a_{e}_{task_id}', nn.Parameter(tensor_prompt(new_pt, self.key_d, ortho=True).to(self.device)))
