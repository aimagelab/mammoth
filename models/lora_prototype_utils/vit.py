...
from models.lora_prototype_utils.loralib.layers import Linear as LoraLinear
...
class Mlp(nn.Module):
    ...
    def __init__(
        ...
        linear_layer = partial(LoraLinear, lora_dropout=0.)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, AB: dict = {}, pre_logits=False):
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    if kwargs['args'].pret == 'in21k_old':
        variant = 'vit_base_patch16_224_v2'
...
def vit_base_patch16_224_v2(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('vit_base_patch16_224_v2', pretrained=pretrained, **dict(model_kwargs, **kwargs))
    return model


def vit_base_patch16_224_prompt_prototype(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    ...
    if 'args' not in kwargs:
        ...
        kwargs['args'] = Namespace(pret='in21k')
    if kwargs['args'].pret == 'in21k_old':
        model = vit_base_patch16_224_v2(pretrained=pretrained, **kwargs)
        return model
    elif kwargs['args'].pret == 'in21k':
        model = _create_vision_transformer('vit_base_patch16_224.augreg_in21k', pretrained=pretrained, **dict(model_kwargs, **kwargs))
    else:
