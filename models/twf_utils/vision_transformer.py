...
class Block(nn.Module):
    ...
    def forward(self, x, returnt='out'):
        ...
        x = old_x + self.drop_path1(self.ls1(x))
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, returnt='out'):
        x = self.patch_embed(x)
        x = self._pos_embed(x)
        x = self.norm_pre(x)
        res = {'MHSA_outputs': [], 'block_outputs': []}
        if self.grad_checkpointing and not torch.jit.is_scripting():
            x = checkpoint_seq(self.blocks, x)
        else:
            if returnt == 'full':
                for b in self.blocks:
                    out = b(x, returnt=returnt)
                    res['MHSA_outputs'].append(out['MHSA_output'])
                    x = out['block_output']
                    res['block_outputs'].append(x)
            else:
                x = self.blocks(x)
        x = self.norm(x)
        res['features'] = x
    ...
    def forward_head(self, res, pre_logits: bool = False, returnt='out'):
        x = res['features'] if returnt == 'full' else res
        if self.global_pool:
    ...
    def forward(self, x, returnt='out'):
        x = self.forward_features(x, returnt=returnt)
        x = self.forward_head(x, returnt=returnt)
        return x
...
@register_model
def vit_base_patch16_224_twf(pretrained=False, **kwargs):
...
@register_model
def vit_small_patch16_224_twf(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
...
@register_model
def vit_tiny_patch16_224_twf(pretrained=False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
