...
def test_configs(capsys):
    ...
    namespace_dict = {arg.split('=')[0]: arg.split('=')[1].replace("'", "") for arg in namespace_args}
    ...
    # - `n_epochs` should be 1 (from CLI arguments)
    # - `batch_size` should be 5 (from config file)
    # - `optim_mom` should be 0.9 (from puridiver's defaults)
    assert namespace_dict['n_epochs'] == '1', f'n_epochs not loaded correctly from CLI. Found {config["n_epochs"]} but expected 1'
    assert namespace_dict['batch_size'] == str(config['batch_size']), f'batch_size not loaded correctly from config file. Found {config["batch_size"]} but expected {config["batch_size"]}'
    assert namespace_dict['optim_mom'] == '0.9', f'optim_mom not loaded correctly from puridiver defaults. Found {config["optim_mom"]} but expected 0.9'
